...
function Edit() {
  ...
  const handleSubmit = function (e) {
    ...
    const requestOptions = {
      ...
      body: JSON.stringify({ title:recipe.title, category: recipe.category, shortDesc:recipe.shortDesc, longDesc:recipe.longDesc  , createDate: new Date().getTime() + ""   })
    };
    fetch('http://localhost:9000/api/recipes/' + recipeId, requestOptions)
  ...
  const fretchItems = async () => {
    const data = await fetch('http://localhost:9000/api/recipes/' + recipeId);
